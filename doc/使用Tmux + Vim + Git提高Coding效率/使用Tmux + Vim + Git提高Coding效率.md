...
关于 Git 的使用，这里不作过多展开，大家可以学习 Git 的官方教程。我个人认为，只要理解了 Git 的**提交 （commit）**、**分支（branch）**、**工作区（workspace）**、**暂存区（staged）**、**本地仓库（local）**、**远程仓库（remote）** 这几个概念，那么就很容易理解 Git 的操作命令以及使用 Git 的开发流程及其思想。
